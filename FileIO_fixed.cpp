...
void saveStackToFile(const std::string& filename, const Stack& mStack) {
    ...
    file << "S st " << count;
    // Stack хранится Top -> Bottom, поэтому выводим в прямом порядке
    cur = mStack.head;
    while (cur != nullptr) {
        file << " " << cur->key;
        cur = cur->next;
...
Stack* loadStackFromFile(const std::string& filename) {
    ...
    // Читаем в обратном порядке (так как в файле хранится Top -> Bottom)
